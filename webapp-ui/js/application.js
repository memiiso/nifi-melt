...
var ua = {
    ...
    init: function () {
        ...
        ua.initNewSQLExpressionDialog();
        //ua.initNewActionDialog();
        ua.initOkDialog();
        ua.initYesNoDialog();

        // configure the grids
        var expressionsGrid = ua.initExpressionsGrid();

        // enable grid resizing
        $(window).resize(function (e) {
            if (e.target === window) {
                expressionsGrid.resizeCanvas();
        ...
        $('#new-condition').on('click', function () {
            ...
            if (ruleId === '') {
                ...
                $('#new-sql-expression').nfeditor('setValue', '');

                // show the dialog
                $('#new-condition-dialog').center().show();
                $('#new-sql-expression').nfeditor('refresh').nfeditor('focus');
            }
        });
        ...
        $('#selected-rule-name').on('blur', function () {
            ...
            var ruleItem = $('#table-list').children('li.selected');
            ...
            if (ua.editable) {
                ...
                var conditionsEditController = expressionsGrid.getEditController();
                ...
                handler: {
                    click: function () {
                        ...
                        $.Deferred(function (deferred) {
                            ...
                            $('#table-list').children('li.selected').addClass('unsaved');
     ...
     */
    initNewSQLExpressionDialog: function () {
        var languageId = 'nfel';
        var editorClass = languageId + '-editor';

        var add = function () {
            var sqlExpression = $('#new-sql-expression').nfeditor('getValue');
        ...
        // create the editor
        $('#new-sql-expression').addClass(editorClass).nfeditor({
     ...
     * @returns {undefined}
     */
    initExpressionsGrid: function () {
        // custom formatter for the actions column
        /*
        var expressionActionFormatter = function (row, cell, value, columnDef, dataContext) {
        ...
        var expressionColumns = [
            {
                ...
                sortable: true,
                cssClass: 'pointer',
                editor: ua.getNfelEditor,
                validator: ua.requiredFieldValidator
            }
        ];
        ...
        if (ua.editable) {
            ...
            expressionColumns.push({
                id: "actions",
                name: "&nbsp;",
                formatter: conditionsActionFormatter,
                resizable: false,
                sortable: true,
                width: 50,
                maxWidth: 50
            ...
            expressionOptions = $.extend({
                editable: true
            }, expressionOptions);
        }

        // initialize the dataview
        var expressionData = new Slick.Data.DataView({inlineFilters: false});
        expressionData.setItems([]);

        // initialize the grid
        var expressionGrid = new Slick.Grid('#selected-rule-conditions', expressionData, expressionColumns, expressionOptions);
        expressionGrid.setSelectionModel(new Slick.RowSelectionModel());

        expressionGrid.onCellChange.subscribe(function (e, args) {
            $('#table-list').children('li.selected').addClass('unsaved');
        });
        expressionGrid.onSort.subscribe(function (e, args) {
            var comparer = function (a, b) {
                return a[args.sortCol.field] > b[args.sortCol.field];
            };
            expressionData.sort(comparer, args.sortAsc);
        });
        expressionGrid.onClick.subscribe(function (e, args) {
            if (ua.editable) {
                // edits the clicked cell
                expressionGrid.gotoCell(args.row, args.cell, true);
            } else {
                ua.showValue(expressionGrid, args.row, args.cell);
            }
        ...
        expressionData.onRowCountChanged.subscribe(function (e, args) {
            expressionGrid.updateRowCount();
            expressionGrid.render();
        });
        expressionData.onRowsChanged.subscribe(function (e, args) {
            expressionGrid.invalidateRows(args.rows);
            expressionGrid.render();
        });

        // hold onto an instance of the grid
        $('#selected-rule-conditions').data('gridInstance', expressionGrid);
    ...
    loadRuleList: function () {
        var ruleList = $.ajax({
            ...
            if (rules && rules.length > 0) {
                ...
                $('#table-list').children('li:visible:first').click();
            } else {
                ua.hideRuleList();
            }
        });
    ...
    saveSelectedRule: function () {
        ...
        return $.ajax({
            ...
            var selectedRuleItem = $('#table-list').children('li.selected');
